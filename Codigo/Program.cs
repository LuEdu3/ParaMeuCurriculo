...
using (StreamWriter escrever = new StreamWriter("lista-de-contatos.txt"))
{
    ...
    if (segue == "sim".ToLower() || segue == "s".ToLower()) //Se for inserido "sim" ou "s" entra no bloco de código
    {
        ...
        foreach (string nome in nomes) //Estrutura de repetição, procura nome dentro de nomes
        {
            ...
            escrever.WriteLine(nome); // Escreve no bloco de notas
    ...
    Console.ResetColor(); //Reseta a cor
    Console.WriteLine("Lista de contatos geradas e salvas no arquivo lista-de-contatos.txt.\n");
}
